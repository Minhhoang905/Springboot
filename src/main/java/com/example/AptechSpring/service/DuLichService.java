...
public class DuLichService {
	...
	public List<DuLichResultDTO> findId(Long id,String DiaDiem, Long idTinh, Long idQuan, Long idXa, String DiaChiCuThe, String Mota) {
		//Cách 1: Dùng Map để nhận kết quả
	    // Ở cách này, kết quả trả về là một danh sách Map, trong đó mỗi Map đại diện cho 1 record từ Database.
	    // Cách này linh hoạt hơn vì không yêu cầu một DTO cụ thể, nhưng việc truy xuất giá trị sẽ phức tạp hơn và dễ gặp lỗi kiểu dữ liệu.

		//List<Map<String, Object>> duLich = dulichRepository.findId(id, DiaDiem, idTinh, idQuan, idXa, DiaChiCuThe, Mota);
		
		
		//Cách 2: Dùng DTO để nhận kết quả
		// Trả về danh sách DTO, mỗi DTO chứa dữ liệu của một record với các getter.	
	    // Cách này an toàn hơn về mặt kiểu dữ liệu và giúp tránh lỗi kiểu dữ liệu khi truy xuất các giá trị trong kết quả.
		List<DuLichResultDTO> duLich = duLichRepository.selectAllInfo(id, DiaDiem, idTinh, idQuan, idXa, DiaChiCuThe, Mota);
			return duLich;			
	}

	//Tạo phương thức để chuẩn bị dữ liệu cho trang home/search
	public DuLichHomepageDTO getDuLichHomepage(DuLich dulich) {
		//Gọi đối tượng từ lớp DuLichHomepageDTO và gán phương thức `getDuLichHomepage` để lấy danh sách
	...
	//Tạo phương thức trả về điều kiện tìm kiếm khi nhấn button `search` ở trang home/search
	...
	public DuLich updateInfoById(Long id, String DiaDiem, Long idTinh, Long idQuan, Long idXa, String DiaChiCuThe, String Mota) {
		//Tạo đối tượng theo entity DuLich, trả về theo `id`, nếu không tìm thấy thì trả về `null`
